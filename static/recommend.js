...
// will be called after clicking on the recommended movies
...
// get details of the movie from the API (based on the name of the movie)
  ...
  $.ajax({
    ...
    success: function(movie){
      if(movie.results.length<1){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(510).fadeOut();
      }
      else{
        $("#loader").fadeIn();
        $('.fail').css('display','none');
        $('.results').delay(1000).css('display','block');
        if ( movie.results[0].original_title== "Jurassic World Dominion")
          {
              var movie_title ="Jurassic World";
          }

          else if ( movie.results[0].original_title== "The Art of 'Spirited Away'")
          {
              var movie_title ="Spirited Away";
          }

          else if ( movie.results[0].original_title== "Guardians of the galaxy")
          {
              var movie_title ="Guardians of the Galaxy";
          }
...
// passing the movie name to get the similar movies from flask
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Sorry! The movie you requested is not in our database. Please try with some other movies. Note the TMDB website might have some send incorrect titles sometimes."){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
        ...
        var movie_array = recs.split('---');
        var arr = [];
        for(const movie in movie_array){
          arr.push(movie_array[movie]);
        }
        get_movie_details(movie_id,my_api_key,arr,movie_title);
      }
    },
    error: function(){
      alert("error in recomendations, try refreshing (or hard refresh) or try starting the system again from the cmd.");
      $("#loader").delay(500).fadeOut();
    },
  }); 
}

// get all the details using the movie id.
...
// passing all the details to python's flask for displaying  movie  using imdb id
function show_details(movie_details,arr,movie_title,my_api_key,movie_id){
  ...
  arr_poster = get_posters(arr,my_api_key);
  
  movie_cast = get_cast(movie_id,my_api_key);
  
  individual_cast = get_individual_cast(movie_cast,my_api_key);
  ...
  details = {
    'title':movie_title,
      ...
      'cast_bdays':JSON.stringify(individual_cast.cast_bdays),
      'cast_bios':JSON.stringify(individual_cast.cast_bios),
      'cast_places':JSON.stringify(individual_cast.cast_places),
      'imdb_id':imdb_id,
      'poster':poster,
      'genres':my_genre,
      'overview':overview,
...
function get_cast(movie_id,my_api_key){
    cast_ids= [];
    cast_names = [];
    cast_chars = [];
    cast_profiles = [];
    ...
    $.ajax({
      ...
      success: function(my_movie){
        ...
        for(var mv_cast in top_cast){
          cast_ids.push(my_movie.cast[mv_cast].id)
          cast_names.push(my_movie.cast[mv_cast].name);
          cast_chars.push(my_movie.cast[mv_cast].character);
          cast_profiles.push("https://image.tmdb.org/t/p/original"+my_movie.cast[mv_cast].profile_path);
        }
      },
      error: function(){
        alert("Invalid Request!");
...
function get_posters(arr,my_api_key){
  var arr_poster_list = []
  for(var m in arr) {
    $.ajax({
      type:'GET',
